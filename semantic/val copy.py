...
from utils.semantic.metrics import mask_iou, multiclass_dice_coeff, batch_intersection_union
from utils.semantic.plots import plot_images_and_masks

from utils.semantic.dataloaders import create_dataloader 
from utils.semantic.metrics import Semantic_Metrics, Metrics, ap_per_class_box
...
def run(
    ...
    metrics = Metrics()
    semantic_metrics = Semantic_Metrics(nc = seg_nc, device = device)
    ...
    for batch_i, (im, targets, gt_masks, paths, shapes) in enumerate(pbar):
        ...
        with dt[2]:
            ...
            # print(len(preds), preds[0].size(), pred_masks.size()) # 32 torch.Size([300, 6]) torch.Size([32, 2, 672, 672])

        # Metrics
        plot_semasks = []  # masks for plotting
        # if training:
        #     semantic_metrics.update(pred_masks, gt_masks)
        # else:
        #     _, _, smh, smw = gt_masks.shape
        #     semantic_metrics.update(F.interpolate(pred_masks, size = (smh, smw), mode = 'bilinear', align_corners = False), gt_masks)

        if plots and batch_i < 3:
            plot_semasks.append(pred_masks.clone().detach().cpu())

        # 遍历 batch 内的图片
        for si, (pred,  pred_mask) in enumerate(zip(preds, pred_masks)):
        # for si, pred in enumerate(preds):
           ...
           # Save/log
            ...
            semantic_mask = torch.flatten(pred_mask, start_dim = 1).permute(1, 0) # class x h x w -> (h x w) x class
            ...
            output_masks = torch.reshape(output_masks.permute(1, 0), (seg_nc, h, w)) # (h x w) x class -> class x h x w
            pred_masks = output_masks.to(device = device) # torch.Size([2, 640, 640])

            # 筛选 classes
            instances_ids = getDataIds(name = 'semantic') 
            stuff_mask = torch.zeros((h, w), device = device)
            check_semantic_mask = False
            for idx, pred_semantic_mask in enumerate(pred_masks):
                category_id = int(idx)
                ...
                if category_id not in instances_ids:
                    ...
                    stuff_mask = torch.logical_or(stuff_mask, pred_semantic_mask) # 逻辑 或
        ...
        #     print(plot_semasks.size())
        print("------------------------ plots images and semantic masks ---------------------")
        if plots and batch_i < 3:
            if len(plot_semasks):
                plot_semasks = torch.cat(plot_semasks, dim=0)
    ...
    pf = '%22s' + '%11i' * 2 + '%11.3g' * 6  # print format 4+2
    ...
    # print(metrics.ap_class_index) 分割和检测的输出是否应该分开 .. 
    ...
    final_metric = mp_bbox, mr_bbox, map50_bbox, map_bbox, miou_sem, fwiou_sem
